...
app.get('/api/payroll/employees', async (req, res) => {
    try {
        // Pure read-only query ΓÇö LEFT JOIN returns employees with or without a profile.
...
// --- Documents / Payslips ---
// GET /api/payroll/payslip/:id  ΓÇö JSON for use by the in-browser print dialog
...
// GET /api/payroll/payslip/:employeeId/monthly?month=&year=  ΓÇö PDF download
...
// GET /api/payroll/payslip/:employeeId/yearly?year=  ΓÇö ZIP of all monthly payslips
...
const startServer = (portToTry: number | string) => {
    const server = app.listen(portToTry)
        .on('listening', () => {
            console.log(`Γ£à Server running at http://localhost:${portToTry}`);
        })
        .on('error', (err: any) => {
...
(async () => {
    ...
    try {
        ...
        console.log(`Γ£à Database connected successfully. Server time: ${result.rows[0].now}`);
    } catch (err: any) {
        console.error('Γ¥î Database connection failed:', err.message);
