...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (user?.role === 'employee' && !employeeId) {
                alert("Session incomplete: Employee ID missing. Please re-login.");
                navigate('/login');
                return;
            }

            try {
                // Pass employeeId if available (for employees) or fallback to 'admin' (for HR/Admins)
                const { data } = await api.get('/attendance/today', {
                    params: { employeeId: employeeId || 'admin' }
                });
                setAttendance(data);
            } catch (error) {
    ...
    const handleCheckIn = async () => {
        const employeeId = currentUser?.employee_id || 'admin';
                    ...
                    <div className="bg-white rounded-[8px] border border-slate-200 shadow-sm p-5 flex flex-col items-center">
                        <div className="w-20 h-20 bg-slate-100 rounded-lg mb-4 flex items-center justify-center overflow-hidden border border-slate-100 shadow-sm p-1">
                            <div className="w-full h-full bg-slate-200 rounded flex items-center justify-center">
                                <span className="text-2xl font-light text-slate-400">{initial}</span>
                            </div>
                        </div>
                        <h2 className="text-[13px] font-bold text-slate-800 text-center">{currentUser?.employee_id || '1'} - {userName}</h2>
