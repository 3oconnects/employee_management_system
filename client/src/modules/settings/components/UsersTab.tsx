...
import {
    ...
    UserCheck, Eye, EyeOff, X, Loader2, Mail
} from 'lucide-react';
...
const UsersTab: React.FC<Props> = ({ users, roles, onRefresh, onNotify }) => {
    ...
    const createUser = async () => {
        if (!newUser.name || !newUser.email || !newUser.password) {
            return onNotify('Please fill in all required fields.', false);
        }
        setSaving(true);
        try {
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-xl text-[12px] font-bold hover:bg-indigo-500 transition-all shadow-sm shadow-indigo-200"
                >
